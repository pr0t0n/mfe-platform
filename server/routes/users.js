...
const safe = (u) => { const { password: _, ...rest } = u; rest.active = !!rest.active; rest.company = rest.company || ''; return rest }
...
router.put('/:id', requireAuth, requireAdmin, (req, res) => {
  ...
  const avatar = name.split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase()
  db.prepare('UPDATE users SET name=?,email=?,role=?,department=?,company=?,avatar=? WHERE id=?')
    .run(name, email.toLowerCase().trim(), role, department, company || '', avatar, req.params.id)
  res.json({ ok: true })
})
